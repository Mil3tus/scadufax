...
def generate_service(port_number, protocol):
    ...
    if (protocol == 'tcp'):
        try:
            ...
            with open('data/tcp.csv') as tcp_content:
                # read line by line
            ...
            try:
                ...
                with open('/usr/share/scadufax/data/tcp.csv') as tcp_content:
                    # read line by line
                    for tcp_line in tcp_content:
                        # create port standard for compare
                        check_port = ',' + str(port_number) + ','
        ...
        try:
            # try to find tcp.csv on the same folder as scadufax binary
            with open('data/udp.csv') as udp_content:
                # read line by line
            ...
            try:
                # try to find tcp.csv on /usr/share scadufax (installed version)
                with open('/usr/share/scadufax/data/udp.csv') as udp_content:
                    # read line by line
                    for udp_line in udp_content:
                        # create port standard for compare
                        check_port = ',' + str(port_number) + ','
